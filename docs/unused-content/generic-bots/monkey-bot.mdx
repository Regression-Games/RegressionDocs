...
The Regression Games SDK has a built-in **Monkey Bot** that automatically determines the applicable keyboard and mouse inputs for the game being tested and randomly performs those actions as the game is running. The SDK allows you to select which of the actions are enabled in order to control/limit the functionality that is exercised by the bot.
