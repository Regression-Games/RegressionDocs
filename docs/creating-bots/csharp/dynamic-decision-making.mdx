...
:::info
If you are using the [[RGAction]](../../integrating-with-unity/defining-actions) attribute to generate your actions, then there are also typed `RGActionRequest` classes generated for that action.  The snippets below provide examples for using both typed and untyped actions.
:::

:::caution
Commands within `ProcessTick` interact with the game state represented by the current tick only.
This contrasts with [Unit-Testing Bots](./unit-testing), which can delay execution of commands for several ticks until expected conditions are met.
:::
