...
Validation Bots share the same APIs as [Dynamic Bots](./dynamic-decision-making), with two important distinctions:
...
For example, running the command `rg.findEntity("MyEntity")` for a Dynamic Bot will return the matching entity or null based on the current tick's game state,
...
These methods are in addition to the APIs defined in the [Dynamic Bots section](./dynamic-decision-making#variables).
