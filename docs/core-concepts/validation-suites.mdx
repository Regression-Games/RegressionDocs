...
Validation Scenarios execute in the same order as they appear in the Validation Suite dashboard, but order does not impact how Scenarios run.
Each Scenario starts at the first tick of the Gameplay Session, and a Suite continues to run remaining Scenarios when one fails.
...
![Screenshot of a scenario](img/validation-suites/scenario-example.png)
