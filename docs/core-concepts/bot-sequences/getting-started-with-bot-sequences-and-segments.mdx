...
After adding the `RGOverlayCanvas` prefab to your scene, click on the small Williward face icon to open the overlay. With the overlay opened, you can see the list of Bot Sequences that are available to you. Each Sequence is represented by a card, with three possible actions:
...
![Sequence Editor in RGOverlayCanvas](img/sequence_editing.gif)
...
<div style={{width: '100%'}}>
<img src={SequenceDeleteDialog} alt="Create Sequence Card" style={{display: 'block', height: '200px', margin: '0 auto'}} />
</div>
...
<img src={ReloadIcon} alt="Create Sequence Card" style={{display: 'block', width: '55px', height: '50px'}} />
</div>
