        ...
        "mouse": [ // Any number of mouse inputs can be provided
            {
                ...
                "screenSize": {"x": ..., "y": ...}, // A width and height indicating the size of the screen, which is used to scale the position of the mouse when the screen size is different from the screen size at the time the inputs were recorded
                ...
                "clickedObjectNormalizedPaths": [...], // An optional list of normalized paths to the objects that were clicked
            }
        ],
...
// The `botAction.data` format
{
    "behaviourFullName": "...", // The full class path of the MonoBehaviour to run
...
```csharp
// The monobehaviour code contained somewhere in your game ruintime code
using UnityEngine;
...
the `RestartGame` Bot Action is used in a build of your game **outside the UnityEditor**, then the default action will be to call 
