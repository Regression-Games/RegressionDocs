...
The Regression Games SDK's `RGOverlayCanvas` prefab is a UI component that will allow you to record and replay Gameplay
Sessions.
...
This can be configured using the `UI Replay Enforcement` dropdown of the `Replay Data Playback Controller` Behaviour.  This Behaviour is located on the `RGOverlayCanvas/Recording Toolbar` GameObject.