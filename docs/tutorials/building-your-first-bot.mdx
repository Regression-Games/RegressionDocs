...
First, let's add the state information. Regression Games marks states for bot consumption using the 
...
to see all the ways you can define state within your game, including grabbing all of the state from a MonoBehaviour.

## Implement actions your bot can take

Now, similar to states, we will tag the actions that a bot can take, using [`[RGAction]`](/integrating-with-unity/defining-actions). In our case, a bot can either move or attack.
