...
you can't find that version). Once the project is loaded, a default scene with bots in a playground should open. If it does not, 
...
of the overlay can be hidden through the settings. If you have multiple scenes where a bot will be used,
place this into each scene. The SDK will ensure that only one instance is present across scenes or on scene transitions.

:::

You can find this prefab by navigating to the **Packages** > **Regression Games Unity Bots** > **Runtime** > **Prefabs** folder,
named as `RGOverlayCanvas`. Drag this overlay onto your scene in the object hierarchy. Make sure that the canvas is at the top of
the hierarchy, so that it is always visible and interactable.
...
Regression Games logo will appear at the bottom of your Scene View. Click this button to see a dropdown for your bots. You
...
First, create a `RegressionGames/Runtime/` folder in your `Assets` folder (these folder and file names are what we recommend, 
but they are not required to be named that way). Then, create a file called `CharacterBotSpawnManager.cs`.
...
This attribute can be applied to any public method that returns a value or any variable within a `MonoBehaviour`.
...
* Note: The state will be accessible to the bot as "CurrentHealth", rather than "GetCurrentHp", as we are overriding
*       the name of the state within the RGState attribute.
*/
