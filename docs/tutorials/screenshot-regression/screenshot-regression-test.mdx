...
Once you have your reference screenshots, you'll need to convert them to Base64 encoding. This allows for easy storage and comparison within your testing framework. Copy and paste the
entire string into the `imageData` field, and the save the file.

After saving the file, zip/compress the `Assets/RegressionGames/Resources/sequences/bot_segments` folder that contains all of the JSON files.

## Running the test

Now that we have our zip file of bot segments, we can run this as a bot sequence! Start up the game, select the hamburger menu icon in the Regression
Games overlay, and select `{project_folder}/Assets/RegressionGames/Resources/sequences/bot_segments.zip`.

Click play, and you should see the bot perform the actions for your test sequence. Before completing, in the logs you should see that it runs
your segment related to visual regression test.
