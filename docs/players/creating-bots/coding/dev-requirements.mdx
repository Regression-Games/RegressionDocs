...
You can read more about how to set up a new AI project and how GitHub is involved [here](../starter-templates).

In this section, we'll walk you through creating code for a newly-created AI - this guide focuses on writing the AI yourself,
but you can alternatively use one of our creation tools such as [Blockly](../blockly) or [Agent Builder](../agent-builder) to generate code for you.
...
[Codespaces](https://github.com/features/codespaces) is the fastest way to get started with AI developemnt for Regression Games.
Codespaces is a cloud IDE offered by GitHub and Microsoft, based on VSCode. 
It allows you to edit your GitHub repositories straight from the GitHub website with just a few clicks and with minimal setup.
...
on free GitHub accounts (~60 hrs free per months).
:::

First, log into your GitHub account and navigate to your AI's repository. 
If you need help finding your repository, you can log into Regression Games and find a link to your AI's repository through the Bot Manager.
...
<CodeSnippets />
Click the Source Control button in the left-hand pane (or open Source Control using the shortcut `Ctrl`+`Shift`+`G`). 
...
There may be messages about adding unstaged changes - you can click **always** for this.
...
That's it! If you were to queue into a match, you should see your AI load and follow its new logic. 
If you're in a match while perform updates like these, you should see your AI disconnect for a brief moment and then reconnect, 
...
Once you're logged in, click the "Create Repl" button and then "Import from GitHub".
...
First, you'll need to generate a personal access token, 
...
Clone your repository from GitHub with "`git clone https://github.com/<Your Repo>`".
...
For JavaScript and TypeScript AIs, you'll need to install [Node.js](https://nodejs.org/en/download) version 16.x (which can be checked by running `node -v`). 
...
If you're not using Git to manage your AI project and therefor haven't cloned one of our starter templates, then you'll need to set up the project yourself.
