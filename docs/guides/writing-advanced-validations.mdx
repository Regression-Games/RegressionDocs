...
this is different from using key combinations during active gameplay
...
Why are we indirectly validating that Imps were hit using stagger effects rather than raw health values?
...
You can repeat the last sequence of steps two more times to achieve this effect.
...
Wrap up the Scenario by validating that all three Imps die at the same time and produce an `FX_ImpDefeat` animation.
