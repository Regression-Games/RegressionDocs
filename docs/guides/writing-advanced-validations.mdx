...
Pressing the `3` key allows her to target a position on the ground; clicking the right mouse button unleashes a rain of arrows.
...
Validate that we've targeted a breakable pot by waiting for a left-mouse event and validating that a red target circle
appears around one of the breakable pots in the scene.
Then, wait for the `1` key to be pressed and validate that it launches a basic arrow attack.
...
Store the `PlayerAvatar0`'s `position.x` as "Last Player Position" and the `PickUpPot`'s `position.y` as "Initial Pot Position".
...
Wait for a left-mouse event to produce a `Click_Feedback` circle on the ground.
Validate that the character moves towards this circle by waiting for her `position.x` to be less than the value we just stored.
...
As soon as the character puts the pot down, her first ability should revert to her basic attack.
...
In these cases, it's best to use the default "any" behaviour than supply a specific count.
...
Wait for a right-mouse event and validate that the `ClientAoeInput` disappears, and that appropriate skill animation plays.
...
We can validate that all three Imps were hit by the AOE skill by asserting three `Hitreact Minor` objects appear in the scene.
...
<figure  className="margin--none margin-bottom--md text--center">
    ...
    <figcaption>Select the Imp <code>Hitreact</code> animation specifically! The player has one too.</figcaption>
</figure>

:::info Building Appropriate Validations

Why are we using stagger effects to indirectly validate successful hits on Imps rather than directly validating that their health values drop?

In this Scenario, we cast the AOE skill three times.
...
Depending on the goals of your Scenario, you may want to repeat these steps for each cast of the AOE skill.
Or, you may simply want to validate that the skill is mechanically sound and eventually results in the decimation of your enemies.
Wrap up the Scenario by validating that all three Imps die at the same time.
