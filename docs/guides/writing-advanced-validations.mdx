...
This will enable the "Start with Direct IP" menu option which we can use to create a lobby and select a character class.
We preview several character classes before locking in the Female Archer so that this Scenario can show examples of
UI components changing over time to reflect the highlighted class.
We end the recording once we successfully load into the dungeon map.
...
Confirming your profile name in Boss Room will automatically close the `ProfilePopup` dialog
and activate the new profile.
