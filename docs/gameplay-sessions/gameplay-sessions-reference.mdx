...
```
~/unity_videos/{Application.productName}/{replay|recording}_{date}_{time}_{currentSessionId}_{referenceSessionId}
```

`currentSessionId` is a unique ID for this particular recording, and `referenceSessionId` is the ID of the Gameplay Session
that this recording was simulated from (or equal to `currentSessionId` if this is a completely new recording not played from an 
existing recording). The `replay` or `recording` prefix indicates whether this is an original recording or a replay from
