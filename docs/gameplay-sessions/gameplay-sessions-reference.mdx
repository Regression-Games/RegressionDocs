...
[options you have configured](/gameplay-sessions/gameplay-sessions-usage#advanced-options) for recording, some objects and state in your scene may not be present in this
...
the `keyFrame` field of the `{tick}.json` file. The possible key frames are:

* `FIRST_FRAME` - Indicates that this tick is the first tick of the gameplay session.
* `SCENE` - Indicates that this tick is the first tick of a scene that is different from the previous tick's scene.
* `GAME_ELEMENT` - Indicates that a visible game element was changed this tick.
* `GAME_ELEMENT_RENDERER_COUNT` - Indicates that a visible game element changed its number of renderers this tick.
* `UI_ELEMENT` - Indicates that a visible UI element was changed this tick.
* `UI_PIXELHASH` - Indicates that the screen pixels changed this tick.
