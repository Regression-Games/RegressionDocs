...
When a Gameplay Session is recorded, all data is stored on disk within two folders inside the following parent
directory (which we will reference as the `sessionDirectory`):

```
~/unity_videos/{Application.productName}/{replay|recording}_{date}_{time}_{currentSessionId}_{referenceSessionId|currentSessionId}
```

`currentSessionId` is a unique ID for this particular session, and if this is a session played back from an 
existing Gameplay Session, `referenceSessionId` is the ID of the Gameplay Session that was used to simulate inputs (otherwise,
the suffix of this path is set to the `currentSessionId`). The `replay` or `recording` prefix indicates whether this 
is an original recording or a replay from an existing recording.

The two folders within `sessionDirectory` are:

* `{sessionDirectory}/screenshots` - A folder with screenshots for each recorded tick of
  the session. Each screenshot is named with the format `{tick#}.jpg`, where `tick#` is the in-game tick from the start
  of the session.
* `{sessionDirectory}/data` - A folder with the state and input data for each recorded
...
(`data.zip` and `screenshots.zip` within the `sessionDirectory` folder).
