...
* `~/unity_videos/{Application.productName}/run_{#}/screenshots` - A folder with screenshots for each recorded tick of
  the session. Each screenshot is named with the format `{tick#}.jpg`, where `tick#` is the in-game tick from the start
  of the session.
* `~/unity_videos/{Application.productName}/run_{#}/data` - A folder with the state and input data for each recorded
