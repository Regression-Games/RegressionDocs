...
A recorded gameplay session can also be played back programmatically, using a 
[Unity Test Runner Play Mode test](https://docs.unity3d.com/Packages/com.unity.test-framework@1.4/manual/index.html).
After creating a Play Mode test file, the `RGTestUtils` class can be used to load and play a recording, which in turn
will also record a new session for that test.

:::info Note

Recording `data.zip` files are saved by default to `~/unity_videos/{Application.productName}/run_{#}/`. Make sure
to copy your recordings into a directory within your project to ensure that they are available for other machines
...
public class RecordingTest
{
    ...
    {
        // Allow error logs to be ignored
        ...
        yield return RGTestUtils.StartPlaybackFromFile(recordingPath, result => saveLocation = result);

        // Print out the recording path for viewing later
        RGDebug.LogInfo("Played back and recorded the test run - saved to " + saveLocation);
        Assert.IsNotNull(saveLocation);
