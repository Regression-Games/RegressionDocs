...
First, add the RGOverlayCanvas prefab to your scene, which is the UI that allows us to record and play gameplay
sessions.
...
If the FPS is set `0`, then the SDK will only record information when a
[**key frame**](/gameplay-sessions/gameplay-sessions-reference#key-frames) is detected.
...
bounds, colliders, rigid bodies, and more on individual armatures, weapons, or other components that are children in the
hierarchy.

#### Include Only On Camera Objects

The **Include Only On Camera Objects** checkbox dictates whether the recorder should collect all objects in the
...
The **Collect State from Behaviours** checkbox dictates whether the recorder should collect fields and property
