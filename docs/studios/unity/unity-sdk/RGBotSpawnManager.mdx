...
TODO: Add a link to the API docs for BotInformation
...
Defines how a bot if spawned into a scene. More specifically, `botInformation` holds the configuration for the bot, 
...
`botInformation` | `BotInformation` | required | The id of the client, name of the bot, and a JSON string containing configuration about your bot.
...
provided by the bot using the `BotInformation.UpdateCharacterConfig<T>(T)` method, which sends a new config back to your bot. 
...
Argument Name | Type | Default | Description
---|---|---|---|
`botToSpawn` | `BotInformation` | required | The id of the client, name of the bot, and a JSON string containing configuration about your bot.
