...
In most RGAction implementations, it is necessary to find GameObjects within the scene based 
on the game state. See the [`RGState`](./RGState) class for defining the game state and the 
[`RGFindUtils`](./RGFindUtils) class for helper functions to locate GameObjects within your scene.
