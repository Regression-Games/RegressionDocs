...
and validate changes in the game state in response to character actions. 
...
To enable this mode, add the following line at the start of your `configureBot` implementation:
...
This is the amount of time a command is allowed to wait/retry before the command fails.
