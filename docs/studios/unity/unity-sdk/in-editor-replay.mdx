...
#### Validations
Shows any validation(s) that the Bot performed on the current tick.  These are normally test cases coded into the Bot to validate that the tick data changed as expected based on prior actions or other factors in the game state.  See [Validation Bots](./creating-bots/validation-bots) for more information.
...
When loading a replay zip, the list of entity types to replay model associations will automatically be updated with any missing entities that have `isStatic=false`.  You can also manually add, edit, or remove assocations at any time.

![configure replay models](configure_replay_models.png)
...
You can edit this asset in the inspector directly or using the `Configure Custom Replay Models` button in the RG Replay Plaback window.
:::

![replay model manager](replay_model_manager.png)

:::tip
While the associations support any game object, it is best to associate just the model for your entity.
