...
For each tick, the Bot Replay Window will show all entities that were tracked as part of the Game State sent to Regression Games.  These are sorted alphabetically with any entities controlled by Regression Games Bots and any active/enabled entities listed first.
...
By default, all 4 checkboxes are enabled for the object of the Bot Instance that the replay data was recorded from.
